...
int n,k;
ll cnt;
void merge(vector*arr,int front,int mid,int back){
    int i=front,j=mid+1,t=1;
    vector *tmp=(vector*)malloc(sizeof(vector));init(tmp);
    while(i<=mid && j<=back){
        if(arr->arr[i].v<=arr->arr[j].v){
            push_back(tmp, arr->arr[i++].v);
        }else{
            push_back(tmp, arr->arr[j++].v);
        }
    }
    while(i<=mid){
        push_back(tmp, arr->arr[i++].v);
    ...
    i=front,t=0;
    while(i<=back){
        arr->arr[i++].v = tmp->arr[t++].v;
        cnt++;
        if(cnt==k){
            printf("%d",arr->arr[i-1].v);
            exit(0);
        }
    }
}
void merge_sort(vector*arr,int front,int back){
    if(front < back){
