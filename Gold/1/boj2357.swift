...
func input(){
    ...
    d = [[Pair]](repeating: [Pair](repeating: Pair(mini: Int.max, maxi: Int.min), count: n+1), count: Int(log2(Double(n))) + 1)
}
func Precomputation(){
    for i in 1...n{
        let t = inputs.readInt()
        d[0][i] = Pair(mini: t, maxi: t)
    }
    // [i ~ i + 2^k) 구간 내에 최솟값 저장
    /// [i ~ i + 2^(k-1)] 의 2개으 구간에서 갖고온 최솟값을 저장
    for i in 1...Int(log2(Double(n))){//log2 행 동안
        for j in 1...(n - (1 << i) + 1){ //log번
            d[i][j].mini = min(d[i-1][j].mini , d[i-1][j + (1 << (i - 1))].mini)
            d[i][j].maxi = max(d[i-1][j].maxi , d[i-1][j + (1 << (i - 1))].maxi)
        }
    }
}
func solve(){
    ...
    for _ in 0..<m{
        ...
        print(min(d[k][a].mini, d[k][b - (1 << k) + 1].mini),max(d[k][a].maxi, d[k][b - (1 << k) + 1].maxi))
